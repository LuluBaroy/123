...
exports.readOne = (req, res, next) => {
	...
	if(!headerAuth){
		...
		models.Users.findOne({ where: { id: req.params.id }, include: [models.Posts, models.Comments, models.Likes, models.PostsReport, models.CommentsReport]})
			.then((users) => {
				if(!users){
					...
					let user = users
					user.url_profile_picture = `https://${req.get('host')}/images/${users.url_profile_picture}`
	...
	if(!headerAuth){
		...
		function deleting () {
			return new Promise((resolve) => {
				models.CommentsReport.update({user_id: 12}, {where: {user_id: req.params.id}})
					.then(() => {
						models.PostsReport.update({user_id: 12}, {where: {user_id: req.params.id}})
							.then(() => {
								models.Likes.update({user_id: 12}, {where: {user_id: req.params.id}})
									.then(() => {
										models.Comments.update({user_id: 12}, {where: {user_id: req.params.id}})
											.then(() => {
												models.Posts.update({user_id: 12}, {where: {user_id: req.params.id}})
													.then(() => resolve())
													.catch(err => res.status(500).json(err))
											}).catch(err => res.status(500).json(err))
									}).catch(err => res.status(500).json(err))
								...
								models.Users.update({ ...req.body, password: password, url_profile_picture: urlProfilePicture, consents: JSON.stringify(consents)}, { where: { id: req.params.id }})
									.then(() => {
										...
										models.Users.findOne({ where: { id: req.params.id }})
											.then(users => {
												let user = users
												user.url_profile_picture = `https://${req.get('host')}/images/${user.url_profile_picture}`
