				...
				if(likes.length > 0){
					...
					for(let k = 0; k < userLiked.length; k++){
						models.Users.findOne({ where: { id: userLiked[k] } })
							.then((user => {
								userInfo.push({id:user.id, username: user.username, url_profile_picture: `https://${req.get('host')}/images/${user.url_profile_picture}`, alt_profile_picture: user.alt_profile_picture})
...
exports.readOne = (req, res, next) => {
	...
	if(!headerAuth){
		...
		let userId = jwtUtils.getUserId(headerAuth);
		models.Posts.findOne({ where: { id: req.params.id }, include: [models.Users, models.Comments, models.Likes]})
			.then((posts) => {
				let post = posts
				post.url_gif = `https://${req.get('host')}/images/${posts.url_gif}`
				if(post.User){
					post.User.url_profile_picture = `https://${req.get('host')}/images/${post.User.url_profile_picture}`
				}
	...
	if(!headerAuth){
		...
		models.Posts.findAll({ include: [models.Users, models.Comments, models.Likes],
			...
			.then(post => {
				let posts = post
				posts.forEach(post => {
					post.url_gif = `https://${req.get('host')}/images/${post.url_gif}`
					post.User.url_profile_picture = `https://${req.get('host')}/images/${post.User.url_profile_picture}`
				})
...
exports.readAllFromUserId = (req, res, next) => {
	...
	if(!headerAuth){
		...
		models.Posts.findAll({ where: { user_id: req.params.user_id }, include: [models.Users, models.Likes, models.Comments]}, {
			order: [
				['id', 'DESC']
			]
		})
